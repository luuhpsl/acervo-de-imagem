...
CENTRAL_ASSET_DIR = os.path.join(PROJECT_ROOT, "template-ia-python-master", "src", "acervo_visual_inteligente", "Icons - Programa")
ASSET_DIR = CENTRAL_ASSET_DIR if os.path.isdir(CENTRAL_ASSET_DIR) else _resource_path("Icons - Programa")
...
def _crash_log_path():
    return os.path.join(APP_DIR, "acervo_crash.log")


def _registrar_crash(titulo, texto):
    try:
...
def _instalar_log_crash():
    def excepthook(exc_type, exc, tb):
        _registrar_crash("EXCECAO NAO TRATADA", "".join(traceback.format_exception(exc_type, exc, tb)))
        try:
            messagebox.showerror("Erro inesperado", f"O programa encontrou um erro e registrou detalhes em:\n{_crash_log_path()}")
        except Exception:
            pass

    def threadhook(args):
        _registrar_crash("EXCECAO EM THREAD", "".join(traceback.format_exception(args.exc_type, args.exc_value, args.exc_traceback)))
        try:
            messagebox.showerror("Erro no processamento", f"O processamento encontrou um erro e registrou detalhes em:\n{_crash_log_path()}")
        except Exception:
            pass
